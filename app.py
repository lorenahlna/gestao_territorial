...
# 🌟 DICIONÁRIO DE COLUNAS TERRITORIAIS (CORRIGIDO)
    ...
    try:
        ...
        if nivel_terr == "Município":
            ...
            if col_filtro_res:
                ...
                mask_res = df_t[col_filtro_res].str.zfill(6).str.startswith(id_datasus_alvo[:6])
            if col_filtro_oco:
                df_t[col_filtro_oco] = df_t[col_filtro_oco].apply(normalizar_codigo)
                mask_oco = df_t[col_filtro_oco].str.zfill(6).str.startswith(id_datasus_alvo[:6])
                    ...
                    if not df_bruto.empty and "Erro" not in df_bruto.columns:
                        ...
                        if nivel_terr == "Município" and "CNES" not in sistema:
                            ...
                            if col_oco and col_res:
                                ...
                                # Aplica zfill(6) para garantir 6 dígitos
                                mask_res = df_tratado[col_res].astype(str).str.replace(".0", "", regex=False).str.strip().str.zfill(6).str.startswith(cod6)
                                mask_oco = df_tratado[col_oco].astype(str).str.replace(".0", "", regex=False).str.strip().str.zfill(6).str.startswith(cod6)
                                ...
                                def classificar_relacao(row):
                                    r = str(row[col_res]).zfill(6).startswith(id_datasus_alvo[:6])
                                    o = str(row[col_oco]).zfill(6).startswith(id_datasus_alvo[:6])
                                ...
                                mask_oco = df_tratado[col_oco].apply(normalizar_codigo).astype(str).str.zfill(6).str.startswith(id_datasus_alvo[:6])
                            ...
                            if (nivel_terr == "Estado" and sistema in BASES_PESADAS) or ("SINAN" in sistema and nivel_terr == "Estado" and len(df_tratado) >= 50000):
                                ...
                                if nivel_terr == "Município" and "CNES" not in sistema and col_res and col_oco:
                                    if "SIH" in sistema:
                                        ...
                                        df_dash = df_tratado[df_tratado[col_oco].astype(str).str.zfill(6).str.startswith(id_datasus_alvo[:6])]
                                    else:
                                        st.info("🏠 **Lente Epidemiológica/Residência:** O painel abaixo foca na saúde dos **MORADORES DA CIDADE**, para o planejamento local.")
                                        df_dash = df_tratado[df_tratado[col_res].astype(str).str.zfill(6).str.startswith(id_datasus_alvo[:6])]
