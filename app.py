# VERSAO_DEFINITIVA_SIH_HOMOLOGADA_V11
...
def aplicar_filtros_imediato(df_t, sistema, nivel_terr, uf, id_datasus_alvo, mes_num, cols_alvo, dt_alvos):
    try:
        ...
        # 🌟 CORREÇÃO METODOLÓGICA FIOCRUZ/BASE DOS DADOS: Ignora sub-filtro mensal para SIH e CNES (já vêm isolados por faturamento)
...
if aba_ativa == "📋 Guia Principal (Extração)":
    ...
    if fonte == "🏥 Saúde (DATASUS)":
        ...
        if submit_button:
            # Trava do "Todos os Meses" só barra a Dengue no SINAN
                        ...
                        with tab3:
                            if (nivel_terr == "Estado" and sistema in BASES_PESADAS) or ("SINAN" in sistema and len(df_tratado) >= 50000):
                                ...
                                if "SIH" in sistema:
                                    ...
                                    with c_sexo:
                                        # 🌟 CORREÇÃO DE SINTAXE DE SESSÃO ANALÍTICA
