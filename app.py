# VERSAO_FINAL_PRODUCAO_SUPER_DASHBOARD_V35
...
# 🌟 DICIONÁRIO DE COLUNAS TERRITORIAIS (Corrigido SIM e SINAN)
def obter_colunas_territoriais(sistema, grupo=None):
    ...
    if "SINASC" in sistema: return {"res": ["CODMUNRES"], "oco": ["CODMUNNASC"]}
...
# 🌟 FILTRO V20 RESTAURADO (INTOCADO PARA O SIH-SP)
...
def tratar_e_traduzir_df(df, sistema):
    ...
    # 🌟 SIM: DICIONÁRIO OFICIAL RECRIADO
    if "SIM" in sistema:
                    ...
                    if not df_bruto.empty and "Erro" not in df_bruto.columns:
                        ...
                        if nivel_terr == "Município" and "CNES" not in sistema:
                            ...
                            if col_oco and col_res:
                                # 🌟 APLICA NORMALIZAR_CODIGO PARA GARANTIR MATCH EXATO E LIMPO
                                ...
                                if nivel_terr == "Município" and "CNES" not in sistema:
                                    if "SIH" in sistema:
                                        if col_oco:
                                            ...
                                            df_dash = df_tratado[df_tratado[col_oco].apply(normalizar_codigo).astype(str).str.startswith(id_datasus_alvo[:6])]
                                    else:
                                        if col_res:
                                            st.info("🏠 **Lente Epidemiológica/Residência:** O painel abaixo foca na saúde dos **MORADORES DA CIDADE**, para o planejamento local.")
                                            df_dash = df_tratado[df_tratado[col_res].apply(normalizar_codigo).astype(str).str.startswith(id_datasus_alvo[:6])]
